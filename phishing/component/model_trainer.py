...
from phishing.entity.artifact_entity import DataTransformationArtifact, ModelTrainerArtifact
...
class HousingEstimatorModel:
    ...
    def predict(self, X):
        ...
        transformed_feature = np.nan_to_num(transformed_feature.astype(np.float32),nan=-1,posinf=1,neginf=-1)
        print("transformed_feature finite",np.all(np.isfinite(transformed_feature)))
        print("transformed_feature nan",np.any(np.isnan(transformed_feature)))
        print("transformed feature dtypes ", transformed_feature.dtype)
        
        # transformed_feature = transformed_feature.replace((np.inf, -np.inf, np.nan), -1).reset_index(drop=True)
        
       # transformed_feature = transformed_feature.astype(np.float64, copy=False)
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self)->ModelTrainerArtifact:
        try:
            ...
            train_avg_precision=metric_info.train_avg_precision,
            test_avg_precision=metric_info.test_avg_precision,
